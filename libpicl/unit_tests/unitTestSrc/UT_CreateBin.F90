...
PROGRAM main
    ...
    use ppiclf_data, only: ppiclf_np, ppiclf_nndist, ppiclf_overlap, ppiclf_filter, ppiclf_n_bins

    use ppiclf_solve, only: ppiclf_solve_Initialize, ppiclf_solve_InitParticle
    ...
    REAL*8    numBins, xmin, xmax, ymin, ymax, zmin, zmax, BinMinLen(3)
    LOGICAL   BinsCreated
    ...
    DO loop = 1,4
        ...
        CALL ppiclf_solve_InitParticle(2,3,0,particlesPerProc, p_parts,filter,nndist)
        ...
        DO i = 1,3
            ...
            BinMaxNum(i) = INT((gridDomain(2,i) - gridDomain(1,i)) / BinMinLen(i))
        END DO
        ...
        WRITE(wf,*) 'Max Numbers of bins', ' based on filter size:', BinMaxNum(1), BinMaxNum(2), BinMaxNum(3)
        WRITE(wf,*) 'Number of Processors, x bins, y bins, z bins,',', total bins, Percent of Processors In Use:' 
        ...
        DO i = 1,100000
            ...
            ! Catches errors
            numBins = ppiclf_n_bins(1)*ppiclf_n_bins(2)*ppiclf_n_bins(3)
            IF(numBins .LT. 1 .OR. numBins .GT. ppiclf_np) BinsCreated = .FALSE.
            WRITE(wf,*) ppiclf_np,ppiclf_n_bins(1), ppiclf_n_bins(2), ppiclf_n_bins(3),numBins , numBins/ppiclf_np*100
            IF(BinMaxNum(1) .EQ. ppiclf_n_bins(1) .AND. BinMaxNum(2) .EQ. ppiclf_n_bins(2) .AND. BinMaxNum(3) .EQ. ppiclf_n_bins(3)) THEN
                WRITE(wf,*) 'Max Number of bins reached based',' on minimum bin lengths!'
                EXIT
            END IF
        END DO
        CLOSE(UNIT=wf)
