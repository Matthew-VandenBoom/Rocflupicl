...
module UT_m_setup
    use mpi
    use UT_m_data
    use ppiclf_m_comm, only: user_particle_MPIH
    ...
    SUBROUTINE UT_setup
        ...
        REAL*8    xmin, xmax, ymin, ymax, zmin, zmax, nndistTemp, rhop

        INTEGER :: rng_seed(8)
        ! type(PPICLF_U_t_particle) particle_setup(PPICLF_LPART)
        ! Hard Coded RNG seed, for reproducibility (at least so long as the rng seed size is static)
        rng_seed = [-432888204, -269353627, -177005610, 851601331, 557211905, -22704965, 646077110, -396079526]
        ...
        IF(nid .EQ. rootProc) THEN 
            DO i = 1,totalParticles
                DO j = 1,3 ! loop through x, y, & z coordinates
                    ...
                    parts(i)%y%pos%vec(j) = gridDomain(1,j) + (gridDomain(2,j) - gridDomain(1,j))*randNum
                    parts(i)%tag%partNum = i
                    parts(i)%tag%cycleNum = 0
                    parts(i)%tag%rankNum = 0
                    ...
                    IF(i .LE. faceParticles) THEN
                        ...
                        IF(i - (k-1)*INT(REAL(faceParticles/3.0)+0.49D0) .LT. INT(REAL(faceParticles/6.0)+1.49D0)) THEN
                                ...
                                parts(i)%y%pos%vec(j) = gridDomain(1,j) + (pdia/2.0)*randNum
                            ELSE
                                ! 1/6 faceParticles on max face
                                parts(i)%y%pos%vec(j) = gridDomain(2,j) - (pdia/2.0)*randNum
                            END IF
                        END IF ! j == k
                    END IF ! i < faceParticles
                    ! Make sure that there are some corner particles!
                    IF(i .GT. faceParticles .AND. i .LE. INT(REAL(faceParticles)*(1.0D0+1.0D0/6.0D0))) parts(i)%y%pos%vec(j) = gridDomain(2,j) - pdia/2.0D0*randNum
                END DO ! j
            END DO ! i 
        END IF 

        ! Share part_y from root processor to all processors
        CALL MPI_BCAST(parts,totalParticles,user_particle_MPIH,rootProc,MPI_COMM_WORLD,ierr)
